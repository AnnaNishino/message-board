...
url = "https://liuyan.people.com.cn/threads/searchIndex?keywords=%E9%9A%94%E7%A6%BB"
...
df.to_csv('data/'+datetime.today().strftime('%Y%m%d')+'.csv', encoding='utf_8_sig')


# In[ ]:


master = pd.read_csv('data/master.csv',index_col=0,lineterminator='\n')


# In[ ]:


master = pd.concat([df,master]).drop_duplicates(subset=['tid'])
master.to_csv('data/master.csv', encoding='utf_8_sig')
